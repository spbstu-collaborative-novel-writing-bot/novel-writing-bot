...
| `HTTP_ADMIN_TOKEN` | Токен для `/admin`, `/admin/api/*` и `GET /users` |
...
`GET /admin` отдает Web-панель администратора. JSON endpoints `/admin/api/overview`, `/admin/api/users`, `/admin/api/novels`, `/admin/api/llm-requests` доступны только с заголовком `X-Admin-Token`.

`GET /users` оставлен как короткий JSON-список пользователей и тоже требует `X-Admin-Token`.
...
Права доступа проверяются по `novel_authors`. У романа может быть несколько владельцев. Системная роль `Admin` дает доступ к админским HTTP endpoint-ам, но не открывает чужие произведения в Telegram.
...
Покрыты регистрация пользователя, создание произведения, проверка доступа, дополнительные владельцы и соавторы, добавление главы, история изменений, статистика романа, optimistic conflict при сохранении главы, создание LLM-запроса со статусом `QUEUED`, Telegram callback-кнопки, WebUI-кнопка, Mini App LLM endpoints, `/healthcheck`, `/users` и админский overview.
...
12. Проверьте `/admin`, `/admin/api/overview` и `GET /users` без токена и с корректным `X-Admin-Token`.
